...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def select(para) :
        para_ = split(lower(remove_punctuation(para)))
        for a in topic :
            for b in para_ :
                if a == b :
                    return True
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for c in valid_words :
        if diff_function(user_word, c, limit) < min_ :
            min_ = diff_function(user_word, c, limit)
            out = c 
    return out 
    # END PROBLEM 5
...
def feline_fixes(start, goal, limit):
    ...
    """
    if limit < 0:
        return limit + 1  # Exceeded limit, return a value greater than limit
    ...
    # Try adding a letter to start
    add_diff = feline_fixes(start, goal[1:], limit - 1)

    # Try removing a letter from start
    remove_diff = feline_fixes(start[1:], goal, limit - 1)

    # Try substituting a letter in start
    substitute_diff = feline_fixes(start[1:], goal[1:], limit - 1)

    # Return the minimum difference among the three options
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)) :
        ...
        for j in range(len(times_per_player[0])-1) :
            a = times_per_player[i][j+1] - times_per_player[i][j]
            q.append(a)
        p.append(q)
    return [words  ,p]
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    for i in words :
        per = 0
        for p in range(len(all_times(game))-1)  :
            if game[1][p+1][i] < game[1][per][i] :
                per = p+1
        result[per].append(word_at(game,i))
    return result
    # END PROBLEM 10
