...
def take_turn(prev_rolls, move_history, goal, game_rules):
    ...
    swine_swap = None if "Swine Swap" not in game_rules else game_rules["Swine Swap"]

    try:
        old_swine_swap = hog.swine_swap
        if not swine_swap:
            hog.swine_swap = lambda score0, score1: False
...
def strategy(name, scores):
    STRATEGIES = {
        ...
        "swine_swap_strategy": hog.swine_swap_strategy,
